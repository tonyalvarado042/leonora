...
create type tipo_grupo    as enum ('familia', 'amigos', 'iglesia', 'otro');
...
create type rol_grupo     as enum ('tutor', 'miembro');
create type estado_miembro as enum ('invitado', 'activo', 'salio');
create type tipo_encargo  as enum ('tarea', 'recordatorio', 'consejo');
create type estado_encargo as enum ('pendiente', 'hecho', 'archivado');
create type tipo_evento   as enum (
  'feriado', 'escolar', 'examen', 'entrega', 'cumpleanos', 'cita', 'viaje', 'personal'
);
create type efecto_evento as enum ('libra_el_dia', 'bloquea_horas', 'solo_avisa');
create type repeticion_evento as enum ('ninguna', 'anual');
create type origen_evento as enum ('manual', 'foto', 'sistema');
...
create table grupos (
  ...
  tipo        tipo_grupo not null default 'familia',
  emoji       text not null default '👥',
  creado_por  uuid not null references personas (id) on delete cascade,
  creado_en   timestamptz not null default now()
);

create index grupos_creador on grupos (creado_por);
...
create index miembros_por_persona on miembros_grupo (persona_id, estado);

-- ---------------------------------------------------------------- encargos

create table encargos (
  id                uuid primary key default gen_random_uuid(),
  de_persona_id     uuid not null references personas (id) on delete cascade,
  para_persona_id   uuid not null references personas (id) on delete cascade,
  titulo            text not null check (length(trim(titulo)) between 1 and 120),
  nota              text,
  fecha             date not null,
  hora_sugerida     time,
  tipo              tipo_encargo not null default 'tarea',
...
create index encargos_para on encargos (para_persona_id, estado, fecha);
create index encargos_de   on encargos (de_persona_id, creado_en desc);

-- La tarea del día se acuerda de qué recado la trajo: así, marcarla aquí se
-- ve allá, y quien la mandó se entera de que ya está.
alter table tareas_dia
  add column encargo_id uuid references encargos (id) on delete set null;

create index tareas_por_encargo on tareas_dia (encargo_id);
...
create table eventos (
  ...
  tipo          tipo_evento not null default 'personal',
  titulo        text not null check (length(trim(titulo)) between 1 and 120),
  descripcion   text,
  ...
  efecto        efecto_evento not null default 'solo_avisa',
  origen        origen_evento not null default 'manual',
...
create index eventos_por_fecha   on eventos (fecha_inicio, fecha_fin);
create index eventos_por_persona on eventos (persona_id);
create index eventos_por_grupo   on eventos (grupo_id);
 ...
 *
 * Es la misma regla que `grupos.ts` en la app, escrita aquí porque es aquí
...
create policy creo_yo on grupos
...
create policy borro_lo_que_cree on grupos
...
create policy mios on encargos
  for select using (
...
create policy los_dos_lados on encargos
  for update using (
    de_persona_id = (select auth.uid()) or para_persona_id = (select auth.uid())
  ) with check (
    de_persona_id = (select auth.uid()) or para_persona_id = (select auth.uid())
...
create policy borra_quien_mando on encargos
...
create policy de_quien_puedo_ver on tareas_dia
  for select using (
    dia_id in (select id from dias where puedo_ver_calendario_de(persona_id))
  );
...
create or replace function crear_familia_al_registrarse()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  nueva uuid;
begin
  insert into grupos (nombre, tipo, emoji, creado_por)
  ...
  insert into miembros_grupo (grupo_id, persona_id, rol, ve_mi_calendario, estado)
...
create trigger al_crear_persona_su_familia
  ...
  for each row execute function crear_familia_al_registrarse();
...
revoke execute on function crear_familia_al_registrarse() from public, anon, authenticated;
