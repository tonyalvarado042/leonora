...
create or replace function claude_graceday.nuevo_codigo(tipo tipo_grupo)
returns text
...
begin
  ...
  loop
    ...
    exit when not exists (select 1 from invitaciones where codigo = intento);
  end loop;
  return intento;
end;
$$;
...
create table invitaciones (
  ...
  creada_por   uuid not null references personas (id) on delete cascade,
...
create index invitaciones_por_correo on invitaciones (email) where aceptada_en is null;
create index invitaciones_por_grupo  on invitaciones (grupo_id);
...
begin
  if new.codigo is null or new.codigo !~ '^[A-Z]{4}-[A-Z0-9]{4}$' then
    select tipo into suyo from grupos where id = new.grupo_id;
  ...
  for insert with check (
    ...
    (persona_id = (select auth.uid())
     and (claude_graceday.tengo_invitacion(grupo_id)
          or exists (select 1 from grupos g
...
create policy mias on invitaciones
  for select using (
    email = lower((select auth.email())) or creada_por = (select auth.uid())
  );

create policy invito_yo on invitaciones
  for insert with check (
